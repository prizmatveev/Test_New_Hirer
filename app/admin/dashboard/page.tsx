  ...
  return <main className="container py-10 space-y-6">
    ...
    <section className="card p-4 overflow-auto space-y-3">
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {filteredApplications.map((a) => <tr key={a.id} className="border-b align-top"><td className="p-2">{a.user.name}<div className="text-secondary">{a.user.email}<br />{a.phone}</div></td><td>{a.job.category}</td><td>{a.job.title}</td><td>{a.status}</td><td><a href={`/api/admin/applications/${a.id}/resume`} className="underline" target="_blank" rel="noopener noreferrer">Download</a></td><td className="p-2"><div className="flex gap-2 flex-wrap">
    ...
    {selected && <div className="fixed inset-0 bg-black/40 grid place-items-center p-4" onClick={() => setSelected(null)}>
      <div className="card p-6 w-full max-w-2xl space-y-3" onClick={(e) => e.stopPropagation()}>
        ...
        <div>
          ...
          <p className="text-sm text-secondary whitespace-pre-wrap">{selected.coverLetter || "—"}</p>
        </div>
        <div className="space-y-2"><h4 className="font-medium">Notes</h4>{selected.notes.length === 0 && <p className="text-sm text-secondary">No notes yet.</p>}{selected.notes.map((n) => <div key={n.id}>• {n.note}</div>)}
